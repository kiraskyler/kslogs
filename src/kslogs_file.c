...
*/
static char *kslogs_file_switch(char *file_path)
{
...
{
    static char *w_log_path_buff = NULL;
    kslogs_file->w_log_path = kslogs_file_switch(kslogs_file->user_log_path);
    if(w_log_path_buff != kslogs_file->w_log_path)
    {
        if(kslogs_file->fd > 0)
            close(kslogs_file->fd);
        kslogs_file->fd = 0;
        w_log_path_buff = kslogs_file->w_log_path;
